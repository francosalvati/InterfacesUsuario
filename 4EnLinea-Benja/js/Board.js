class Board {
    ...
    draw(ctx, column) {
        ...
        for (let row = 0; row < this.rows; row++) {
            for (let col = 0; col < this.cols; col++) {
                ...
                // Si hay una imagen asociada a esta celda, dibújala
                const image = new Image();
                image.src = './images/4enlinea/piezaTablero.png';
                if (image) {
                    ctx.drawImage(image, x, y, this.cellSize, this.cellSize);
                } else {
                    // Si no hay imagen, dibuja un rectángulo con el color actual
    ...
    drawContainer(ctx) {//dibuja los containers de los discos
        const containerColor = 'rgba(0, 0, 0, .2)'; // Color negro transparente (ajusta la opacidad según tus preferencias)
